 ...
 *   mono (OpenCode terminal aesthetic); `anthropic` resolves to serif.
 *   Other named color presets fall back to mono unless they list a
 *   different choice. Mirrors how `radius: 'default'` defers to a
 *   per-preset hint.
 * - `mono` - monospace (Berkeley Mono / IBM Plex Mono), the project's
 *   OpenCode default. Tabular figures are enabled globally so numeric
 *   columns align like a terminal.
 * - `sans` - humanist sans (Public Sans), the traditional UI fallback.
 * - `serif` - editorial serif (Lora + CJK fallbacks), the Anthropic
 *   preset's signature typography. Inherits across the whole UI;
 *   monospace contexts keep their own family via Tailwind preflight
 *   and `.font-mono`.
 */
 ...
 * `resolveThemeFont` default of `mono`. The shipped `default` preset
 * opts into mono so the OpenCode terminal aesthetic is the out-of-the-box
 * experience; the `anthropic` preset keeps the editorial Lora serif.
 */
...
export function resolveThemeFont(
  ...
  if (font === 'default') {
    return PRESET_DEFAULT_FONT[preset] ?? 'mono'
  }
  return font
}
